...
int main(int argc, char* argv[])
{
	...
	for (unsigned long i = 0; i < 50; i++)
	{
		...
		#pragma omp parallel for default(none) private(j) shared(Arr1, N) schedule(guided, N / 600)
		for(j = 0; j < N; j++)
		{
		...
		#pragma omp parallel for default(none) private(j) shared(Arr2, Arr2_copy, N) schedule(guided, N / 600)
		for(j = N / 2 - 1; j > 0; j--)
		{
		...
		#pragma omp parallel for default(none) private(j) shared(Arr2, Arr1, N) schedule(guided, N / 600)
		for (j  = 0; j < N / 2; j++)
		{
	...
	long double Sum = 0;
	int j;
	#pragma omp parallel for default(none) private(j) shared(Arr2, N, Sum) schedule(guided, N / 600)
	for (j = 0; j < N / 2; j++){
	...
	fprintf(f, "schedule(guided, N / 600): N=%ld | sum=%Lf Milliseconds passsed: %ld\n", N, Sum, TimeDelta_ms);
	fclose(f);
